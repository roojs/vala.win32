...
const int ID_CLICK = 100;
const int ID_EDIT = 101;
const int ID_LISTBOX = 102;
const int ID_COMBO = 103;
const int ID_SCROLL = 104;
const int ID_LISTVIEW = 105;
const int ID_TREEVIEW = 106;
const int ID_TAB = 107;
const int ID_TOOLBAR = 108;
const int ID_MONTHCAL = 109;
const int ID_DATETIME = 110;
const int SCROLL_MAX = 100;

const int M = 20;
const int PAD = 14;
const int GROUP_Y = 76;
const int LEFT_W = 404;
const int RIGHT_X = 436;
const int RIGHT_W = 484;
const int STATUS_Y = 500;
...
public static int main (string[] args) {
	...
	var name_edit = new Win32.Edit (frame, lx + 52, ly - 2, left_inner_w - 52, 26, ID_EDIT);
	name_edit.text = "Hello, Edit";

	var click_btn = new Win32.Button (frame, lx, ly + 34, 148, 32, ID_CLICK, "Click me");

	new Win32.Label (frame, lx, ly + 72, 40, 20, "List");
	var list_box = new Win32.ListBox (frame, lx, ly + 92, left_inner_w, 64, ID_LISTBOX);
	...
	var combo = new Win32.ComboBox (frame, lx, ly + 184, left_inner_w, 72, ID_COMBO);
	...
	var scroll = new Win32.ScrollBar (
		frame, lx, ly + 284, left_inner_w, 24, ID_SCROLL, 0, SCROLL_MAX, 40
	);
	...
	var list_view = new Win32.ListView (frame, rx, ry, panel_w, list_h, ID_LISTVIEW);
	...
	var tree_view = new Win32.TreeView (frame, rx, row2_y, tree_w, tree_h, ID_TREEVIEW);
	...
	var month_cal = new Win32.MonthCalendar (
		frame, rx + tree_w + cal_gap, row2_y, cal_w, tree_h, ID_MONTHCAL
	);

	var tab_control = new Win32.TabControl (frame, rx, tab_y, panel_w, tab_h, ID_TAB);
	tab_control.add_page ("Overview");
	tab_control.add_page ("Details");
	tab_control.add_page ("Log");

	var toolbar = new Win32.Toolbar (frame, rx, footer_y, panel_w - 236, 28, ID_TOOLBAR);

	new Win32.Label (frame, rx + panel_w - 224, footer_y + 2, 40, 20, "Date");
	var date_pick = new Win32.DateTimePicker (
		frame, rx + panel_w - 180, footer_y, 180, 24, ID_DATETIME
	);
	var tool_tips = new Win32.ToolTips (frame, rx + panel_w - 180, footer_y, 180, 24, ID_DATETIME + 1);
