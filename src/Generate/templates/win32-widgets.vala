...
private void* control_font_default_cache;

private void* control_font_default() {
	if (control_font_default_cache == null) {
		control_font_default_cache = get_stock_object(GetSTOCKOBJECTFLAGS.DEFAULT_GUI_FONT);
	}
	return control_font_default_cache;
}
...
public class WidgetDispatch {
	...
	public static void debug_dump_registry() {
		...
		for (int i = 0; i < wm_command_count; i++) {
			stderr.printf(
				"  [%d] id=%d kind=%d\n",
				i, wm_command_entries[i].wid, (int) wm_command_entries[i].kind
			);
		}
...
private int wm_command_find(int wid) {
	for (int i = 0; i < wm_command_count; i++) {
		if (wm_command_entries[i].wid == wid) {
			return i;
		}
	}
	return -1;
...
private int wm_command_slot(int wid) {
	wm_command_registry_ensure();
	var idx = wm_command_find(wid);
	if (idx < 0) {
		idx = wm_command_count++;
		wm_command_entries[idx].wid = wid;
	}
	return idx;
}

private void wm_command_register_button(Button button) {
	var idx = wm_command_slot(button.WID);
...
private void wm_command_register_edit(Edit edit) {
	var idx = wm_command_slot(edit.WID);
...
private void wm_command_register_list_box(ListBox list_box) {
	var idx = wm_command_slot(list_box.WID);
...
private void wm_command_register_combo_box(ComboBox combo_box) {
	var idx = wm_command_slot(combo_box.WID);
...
private bool wm_command_dispatch(ulong w_param) {
	...
	case WmCommandKind.MENU:
		...
		wm_command_menu_bars[idx].activated(wid);
		return true;
	}
	return false;
}
...
public class Window {
	...
	public Window(
		...
		registered_class_name = WideString(class_name);
		var wide_class = registered_class_name.ptr;
