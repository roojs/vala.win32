...
namespace Generate {
	public class VapiEmitter : Object {
		...
		void reset_shard () {
			...
			this.shard_basename = "";
			this.shard_namespace = "";
		}

		/** One vapi shard: namespace Win32.Ui.* { … } */
		public string emit_shard (Parse.ApiFileEntry entry) {
			this.reset_shard ();
			this.shard_namespace = NameMapper.vala_namespace_from_basename (entry.basename);
			...
			this.buffer.append ("}\n");
			if (this.string_footer.len > 0) {
				this.buffer.append ("\n");
				this.buffer.append_printf ("namespace %s {\n", this.shard_namespace);
				this.buffer.append (this.string_footer.str);
				this.buffer.append ("}\n");
			}
		...
		void emit_string_constant (Parse.Constant c, string vala_name) {
			...
			this.string_footer.append (
				"\t[CCode (array_length = false, array_null_terminated = true)]\n"
			);
			this.string_footer.append_printf ("\tpublic const uint16[] %s = {\n", vala_name);
			for (int i = 0; i < c.ValueText.length; i++) {
				this.string_footer.append_printf ("\t\t'%c', ", c.ValueText[i]);
			}
			this.string_footer.append ("0\n\t};\n\n");
		}
