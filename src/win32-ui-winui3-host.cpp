...
run_winui3_application ()
{
	try {
		...
		winui3_logf (
			"[winui3] bootstrap OK (SDK 0x%08x tag %ls noop)\n",
		...
		const auto deploy_result =
			winrt::Microsoft::Windows::ApplicationModel::WindowsAppRuntime::
				DeploymentManager::Initialize ();
		winui3_logf (
			"[winui3] DeploymentManager::Initialize status %u\n",
			(unsigned) deploy_result.Status ());
		if (deploy_result.Status ()
			!= winrt::Microsoft::Windows::ApplicationModel::WindowsAppRuntime::
				DeploymentStatus::Ok) {
			winui3_log_hresult_utf8 (
				"DeploymentManager::Initialize",
				deploy_result.ExtendedError ());
			return (int) deploy_result.ExtendedError ().value;
		}
...
/* Layer 0 (cf233c0): bootstrap only — never calls sparse / package identity. */
namespace hello_layer {
...
/* Hello-style bootstrap; StackPanel + two TextBlocks (no sparse / embed). */
namespace unpackaged_widgets_layer {

struct LabelDemoApp : ApplicationT<LabelDemoApp>
{
	LabelDemoApp () = default;

	void
	OnLaunched (LaunchActivatedEventArgs const&)
	{
		m_window = Window ();
		m_window.Title (L"vala.win32 WinUI3 unpackaged");

		StackPanel panel;
		panel.Spacing (8);
		panel.Padding (Thickness{16});

		TextBlock title;
		title.Text (L"WinUI3 unpackaged widget");
		title.FontSize (20);

		TextBlock subtitle;
		subtitle.Text (L"StackPanel + 2 labels — no sparse");

		panel.Children ().Append (title);
		panel.Children ().Append (subtitle);
		m_window.Content (panel);
		m_window.Activate ();
		winui3_logf ("[winui3] OnLaunched complete (unpackaged=1)\n");
	}
...
static int
run_unpackaged_widgets ()
{
	try {
		winui3_setup_stdio ();
		SetUnhandledExceptionFilter (winui3_unhandled_exception_filter);
		winui3_logf ("[winui3] unpackaged widgets path (no sparse)\n");
		init_apartment (apartment_type::single_threaded);
		...
		winui3_log_hresult_utf8 ("run_unpackaged_widgets", ex.code ());
		if (!ex.message ().empty ()) {
			winui3_logf ("[winui3] message: ");
			winui3_log_wide (ex.message ().c_str ());
			winui3_logf ("\n");
		...
		winui3_log_error ("run_unpackaged_widgets unknown error");
		return -1;
	}
}

} /* unpackaged_widgets_layer */
...
winui3_run_hello_window (void)
{
	try {
		...
		const HRESULT bootstrap_hr = MddBootstrapInitialize2 (
			...
			MddBootstrapInitializeOptions_None);
		if (FAILED (bootstrap_hr)) {
			hello_layer::show_bootstrap_failure (bootstrap_hr);
			return (int) bootstrap_hr;
		}
		...
		Application app{ nullptr };
		Application::Start ([&app] (auto&&) {
			app = winrt::make<hello_layer::HelloApp> ();
		});
		...
		swprintf (
			...
			(unsigned) code);
		hello_layer::show_error_message (buf);
		return (int) code;
	} catch (...) {
		hello_layer::show_error_message (L"WinUI3 unknown error");
		return -1;
	}
}
...
{
	if (getenv ("WINUI3_UNPACKAGED_WIDGETS") != nullptr) {
		return unpackaged_widgets_layer::run_unpackaged_widgets ();
	}
